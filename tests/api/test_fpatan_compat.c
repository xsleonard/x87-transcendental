...
int main(void)
{
    x87_fpatan *ctx = x87_fpatan_create();
    assert(ctx);
    ...
    for (unsigned i = 0; i < 3; i++)
        for (unsigned r = 0; r < 4; r++) {
            ...
            assert(x87_fpatan_evaluate(ctx, one, one, (x87_fpatan_round)r, pcs[i], &out) ==
                   X87_FPATAN_OK);
            int up = r == X87_FPATAN_RN || r == X87_FPATAN_RU;
            assert(out.value.se == 0x3ffe &&
                   out.value.sig == UINT64_C(0xc90fdaa22168c234) + (unsigned)up);
            assert(out.c1 == up && out.exceptions == 32);
            assert(x87_fpatan_evaluate(ctx, zero, one, (x87_fpatan_round)r, pcs[i], &out) ==
                   X87_FPATAN_OK);
    ...
    assert(x87_fpatan_evaluate(ctx, one, one, (x87_fpatan_round)4, 64, &out) ==
           X87_FPATAN_BAD_ARGUMENT);
    ...
    assert(out.value.se == 0xffff && out.value.sig == UINT64_C(0xc000000000000000) &&
           out.exceptions == 1);
    ...
    static const struct {
        ...
        uint64_t sig[4];
        unsigned char c1[4];
    } regression[] = {{{0x81fb, UINT64_C(0xefdb5b8261893554)},
                       {0x81fc, UINT64_C(0xe405a042f0f267a9)},
                       0xc000,
                       {UINT64_C(0xaa12d89439151ac8),
                        UINT64_C(0xaa12d89439151ac9),
                        UINT64_C(0xaa12d89439151ac8),
                        UINT64_C(0xaa12d89439151ac8)},
                       {0, 1, 0, 0}},
                      {{0x7254, UINT64_C(0xd967009cc2d0386c)},
                       {0x725a, UINT64_C(0xfdf8e8fd4375a221)},
                       0x3ff8,
                       {UINT64_C(0xdb2000abf7b8e333),
                        UINT64_C(0xdb2000abf7b8e333),
                        UINT64_C(0xdb2000abf7b8e334),
                        UINT64_C(0xdb2000abf7b8e333)},
                       {0, 0, 1, 0}},
                      {{0x0f13, UINT64_C(0xfd28b6282f47299d)},
                       {0x0f15, UINT64_C(0xd52fc1d0ff6458f0)},
                       0x3ffd,
                       {UINT64_C(0x93c1b902bf7a2df1),
                        UINT64_C(0x93c1b902bf7a2df0),
                        UINT64_C(0x93c1b902bf7a2df1),
                        UINT64_C(0x93c1b902bf7a2df0)},
                       {1, 0, 1, 0}}};
    for (unsigned j = 0; j < sizeof(regression) / sizeof(regression[0]); j++)
        for (unsigned i = 0; i < 3; i++)
            for (unsigned r = 0; r < 4; r++) {
                assert(
                    x87_fpatan_evaluate(
                        ctx, regression[j].y, regression[j].x, (x87_fpatan_round)r, pcs[i], &out) ==
                    X87_FPATAN_OK);
    ...
    x87_fpatan_destroy(NULL);
    puts("PASS library API and error contract, all RC/PC; no hardware");
    return 0;
}

#!/bin/sh
# Compiler-free h307 narrow shared-sine fractional discriminator capture.
set -eu
cd "$(dirname "$0")"
...
INPUT=inputs/constraint_trig_narrow_sine_fraction_h307.txt
OUT=${1:-trig-narrow-sine-fraction-out}
mkdir -p "$OUT"

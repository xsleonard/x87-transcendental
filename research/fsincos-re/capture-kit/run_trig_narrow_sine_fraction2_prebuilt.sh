#!/bin/sh
# Compiler-free h314 second narrow shared-sine fractional capture.
set -eu
cd "$(dirname "$0")"
...
INPUT=inputs/constraint_trig_narrow_sine_fraction2_h314.txt
OUT=${1:-trig-narrow-sine-fraction2-out}
mkdir -p "$OUT"

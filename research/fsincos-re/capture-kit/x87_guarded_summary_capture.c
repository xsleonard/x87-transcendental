/* H1672: guarded at-most-one-attempt FSIN/FCOS ES/B observation.
 *
 ...
 * attempt at most ONE transcendental, FXSAVE before FWAIT, then observe SIGFPE.
 * Linux may retry #MF without SIGFPE when flags/masks identify no exception.
 * If restored U=0 with nonzero ES/B, record BEFORE ONLY and execute neither
 * FSIN/FCOS nor FWAIT. ATTEMPTED=0 is an observation gap, not a successful gate
 * prediction. Cleanup uses non-waiting FNCLEX; captured before state is intact.
...
#error "H1672 requires the authorized Linux x86-64 research host"
#endif
#include <signal.h>
#include <stddef.h>
#include <ucontext.h>
...
volatile sig_atomic_t h1672_after_valid;
...
extern void h1672_once_sin(struct fxsave_area *);
extern void h1672_once_cos(struct fxsave_area *);
extern unsigned char h1672_sin_site[], h1672_sin_wait[], h1672_sin_resume[];
extern unsigned char h1672_cos_site[], h1672_cos_wait[], h1672_cos_resume[];
...
__asm__(
    ".text\n"
    ".p2align 4\n"
    ".globl h1672_once_sin,h1672_sin_site,h1672_sin_wait,h1672_sin_resume\n"
    ".type h1672_once_sin,@function\n"
    "h1672_once_sin:\n"
    "h1672_sin_site:\n\tfsin\n\tfxsave64 (%rdi)\n"
    "\tmovl $1,h1672_after_valid(%rip)\n"
    "h1672_sin_wait:\n\tfwait\n"
    "h1672_sin_resume:\n\tfnclex\n\tret\n"
    ".size h1672_once_sin,.-h1672_once_sin\n"
    ".p2align 4\n"
    ".globl h1672_once_cos,h1672_cos_site,h1672_cos_wait,h1672_cos_resume\n"
    ".type h1672_once_cos,@function\n"
    "h1672_once_cos:\n"
    "h1672_cos_site:\n\tfcos\n\tfxsave64 (%rdi)\n"
    "\tmovl $1,h1672_after_valid(%rip)\n"
    "h1672_cos_wait:\n\tfwait\n"
    "h1672_cos_resume:\n\tfnclex\n\tret\n"
    ".size h1672_once_cos,.-h1672_once_cos\n"
);
...
int main(void)
{
    ...
    while (fgets(line, sizeof line, stdin)) {
        ...
        fault_seen = fault_kind = fault_code = fault_trap = h1672_after_valid = 0;
        unsigned actual_sw = load_u16(before.bytes + 2);
        unsigned actual_cw = load_u16(before.bytes);
        int attempted = !!(actual_sw & ~actual_cw & 0x3fu) || !(actual_sw & 0x8080u);
        /* Decide from the observed restored image, not requested ES/B. This
         * guard prevents the known synthetic-state kernel retry path. It does
         * not prove arbitrary hardware/spurious-trap behavior cannot occur. */
        if (!attempted) {
            __asm__ volatile("fnclex" ::: "memory");
        } else if (!strcmp(instruction, "fsin")) {
            expected_site = (uintptr_t)h1672_sin_site;
            expected_wait = (uintptr_t)h1672_sin_wait;
            expected_resume = (uintptr_t)h1672_sin_resume;
            armed = 1;
            h1672_once_sin(&after);
        } else {
            expected_site = (uintptr_t)h1672_cos_site;
            expected_wait = (uintptr_t)h1672_cos_wait;
            expected_resume = (uintptr_t)h1672_cos_resume;
            armed = 1;
            h1672_once_cos(&after);
        }
        armed = 0;
        if (fault_seen) {
        ...
        printf("CASE=%s INSN=%s MODE=%s PC=%s MASKS=%02x DEPTH=%u CC=%04x FLAGS=%02x SUMMARY=%04x EMPTY=%u REQ_SW=%04x"
               " ATTEMPTED=%d A_VALID=%d FAULT=%d FAULT_AT=%d SI_CODE=%d TRAP=%d",
            case_id, instruction, mode, precision, masks, depth, cc, flags, summary, empty, sw,
            attempted, h1672_after_valid, fault_seen, fault_kind, fault_code, fault_trap);

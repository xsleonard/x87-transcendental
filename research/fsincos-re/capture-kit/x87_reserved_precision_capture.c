/* H1706: isolated reserved-PC observation harness; NOT YET FROZEN OR EXECUTED.
 * The H1654 once-only sites/handler and prestate checks below are unchanged.
 * Accept pc01 in addition to the standard PCs, but require every exception
 * masked and no pending exception. No warmup, selftest, retry or timing mode.
 * Compile/static inspection is not permission to run unfrozen inputs.
 */
/* H1654: one-attempt FSIN/FCOS capture with deferred and pending #MF state.
 *
 * Input: case insn mode pc masks_hex depth cc_hex flags_hex pending empty se sig
 * Load under all masks, restore the specified coherent prestate, verify it,
...
#error "H1654 requires the authorized Linux x86-64 research host"
#endif
#include <signal.h>
#include <stddef.h>
#include <ucontext.h>
...
volatile sig_atomic_t h1654_after_valid;
...
extern void h1654_once_sin(struct fxsave_area *);
extern void h1654_once_cos(struct fxsave_area *);
extern unsigned char h1654_sin_site[], h1654_sin_wait[], h1654_sin_resume[];
extern unsigned char h1654_cos_site[], h1654_cos_wait[], h1654_cos_resume[];
...
__asm__(
    ".text\n"
    ".p2align 4\n"
    ".globl h1654_once_sin,h1654_sin_site,h1654_sin_wait,h1654_sin_resume\n"
    ".type h1654_once_sin,@function\n"
    "h1654_once_sin:\n"
    "h1654_sin_site:\n\tfsin\n\tfxsave64 (%rdi)\n"
    "\tmovl $1,h1654_after_valid(%rip)\n"
    "h1654_sin_wait:\n\tfwait\n"
    "h1654_sin_resume:\n\tfnclex\n\tret\n"
    ".size h1654_once_sin,.-h1654_once_sin\n"
    ".p2align 4\n"
    ".globl h1654_once_cos,h1654_cos_site,h1654_cos_wait,h1654_cos_resume\n"
    ".type h1654_once_cos,@function\n"
    "h1654_once_cos:\n"
    "h1654_cos_site:\n\tfcos\n\tfxsave64 (%rdi)\n"
    "\tmovl $1,h1654_after_valid(%rip)\n"
    "h1654_cos_wait:\n\tfwait\n"
    "h1654_cos_resume:\n\tfnclex\n\tret\n"
    ".size h1654_once_cos,.-h1654_once_cos\n"
);
...
int main(void)
{
    ...
    while (fgets(line, sizeof line, stdin)) {
        ...
        unsigned masks, depth, cc, flags, pending, empty, se;
        unsigned long long sig;
        ++line_number;
        if (line[0] == '#' || line[0] == '\n') continue;
        int fields = sscanf(line, "%31s %15s %7s %7s %x %u %x %x %u %u %x %llx %c",
            case_id, instruction, mode, precision, &masks, &depth, &cc, &flags,
            &pending, &empty, &se, &sig, &extra);
        ...
        if (fields != 12 || (strcmp(instruction, "fsin") && strcmp(instruction, "fcos"))
            ...
            || masks != 0x3f || pending != 0 || depth < 1 || depth > 8 || empty > 1 || pending > 1 || se > 0xffff
            || (cc & ~0x4700u) || (flags & ~0x007fu) || ((flags & 0x40) && !(flags & 1))
            || pending != !!(flags & ~masks & 0x3f)) {
        ...
        uint16_t sw = (uint16_t)((top << 11) | cc | flags | (pending ? 0x8080 : 0));
         ...
         * specified input. No failed row is silently repaired or retried. */
        ...
        fault_seen = fault_kind = fault_code = fault_trap = h1654_after_valid = 0;
        if (!strcmp(instruction, "fsin")) {
            expected_site = (uintptr_t)h1654_sin_site;
            expected_wait = (uintptr_t)h1654_sin_wait;
            expected_resume = (uintptr_t)h1654_sin_resume;
            armed = 1;
            h1654_once_sin(&after);
        } else {
            expected_site = (uintptr_t)h1654_cos_site;
            expected_wait = (uintptr_t)h1654_cos_wait;
            expected_resume = (uintptr_t)h1654_cos_resume;
            armed = 1;
            h1654_once_cos(&after);
        }
        armed = 0;
        if (fault_seen) {
        ...
        printf("CASE=%s INSN=%s MODE=%s PC=%s MASKS=%02x DEPTH=%u CC=%04x FLAGS=%02x PENDING=%u EMPTY=%u"
               " A_VALID=%d FAULT=%d FAULT_AT=%d SI_CODE=%d TRAP=%d",
            case_id, instruction, mode, precision, masks, depth, cc, flags, pending, empty,
            h1654_after_valid, fault_seen, fault_kind, fault_code, fault_trap);

#!/bin/sh
# Focused h221 standalone-FSIN/paired-FSINCOS causal-tree capture.
set -eu
cd "$(dirname "$0")"
...
INPUT=inputs/constraint_table_fadd_tree_h221.txt
OUT=fadd-tree-out
mkdir -p "$OUT"
...
tar czf fadd-tree-capture.tar.gz "$OUT"
echo "DONE: fadd-tree-capture.tar.gz"

#!/bin/sh
# Focused h224 standalone-FSIN/paired-FSINCOS final-tree capture.
set -eu
cd "$(dirname "$0")"
...
INPUT=inputs/constraint_table_fadd_tree_h224.txt
OUT=fadd-tree-final-out
mkdir -p "$OUT"
...
tar czf fadd-tree-final-capture.tar.gz "$OUT"
echo "DONE: fadd-tree-final-capture.tar.gz"

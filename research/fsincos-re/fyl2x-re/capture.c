/* Public two-operand FYL2X/FYL2XP1 capture harness. No mathematical model here.
 * Exactly one FYL2X/FYL2XP1 per accepted line; never runs a warmup or self-test.
 * A persistent external guard must validate/reserve the WHOLE input first.
 * Input: id op rc pc y_se y_sig x_se x_sig (hex encodings, decimal PC).
 ...
 * retained rather than silently attributing load exceptions to FYL2X/FYL2XP1.
 */
#include <stdint.h>
#include <stdio.h>
#include <string.h>
...
int main(int argc,char **argv)
{
    ...
    if(argc!=1) return 2;
    char line[256],id[64],op[16],rc[4],extra; unsigned pc,ys,xs;
    unsigned long long ym,xm; unsigned long count=0;
    while(fgets(line,sizeof(line),stdin)) {
        if(sscanf(line,"%63s %15s %3s %u %x %llx %x %llx %c",
                  id,op,rc,&pc,&ys,&ym,&xs,&xm,&extra)!=8 ||
        ...
        printf("%s %s %s %u %04x %016llx %04x %016llx %04x %04x %04x %04x %016llx\n",
               id,op,rc,pc,ys,ym,xs,xm,actual,before,after,se,(unsigned long long)sig);

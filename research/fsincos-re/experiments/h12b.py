...
def CENT(row, eprec):
    """table entry at given storage precision: 68 native, or RN/trunc to k bits"""
    s,sig,E=C68(row)
    if eprec==68: return (s,sig,E)
    b=sig.bit_length(); k=abs(eprec); sh=b-k
    if sh<=0: return (s,sig,E)
...
def run(P, wide6, narrow6, combine, EPREC=68):
    globals()['EPREC']=EPREC
    fmul,fadd=mk(P)
    def S_poly(a, six):
        asq=fmul(a,a)
    ...
    for i in idxs:
        ...
        if rf<0.25:
            ...
            sinT=CENT(SINROW[b],globals()["EPREC"]); cosT=CENT(COSROW[b],globals()["EPREC"])
            a=fadd(rv,(1,b,-6))
            S,asq=S_poly(a,six)
            t=C_delta(asq,six)
...
for P in (68,80):
    for ep in (64,-64,65,-65,66,67,68):
        mp,np_,mt,nt=run(P,True,False,1,EPREC=ep)
        print(f"P={P} entprec={ep:+d}: tab {mt[0]}+{mt[1]}/{nt}")

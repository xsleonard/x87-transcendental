#!/usr/bin/env python3
"""h559: finer zoned selector (XD24, free tau coefficient) — j = round(4*tau*4)/... precisely:
...
h559_zoned_selector.json for blind use on other combs).
"""
import json
import sys
...
def main():
    ...
    print("q distribution over zones:", qcount)
    dump = {str(kk): (q, a, b)
            for kk, (c, q, a, b) in fitted.items()
            if a is not None}
    with open("h559_zoned_selector.json", "w") as fh:
        json.dump(dump, fh)
    print("fitted tables -> h559_zoned_selector.json "
          f"({len(dump)} zones)")

#!/usr/bin/env python3
"""Table region: S-chain associativity x rounding-mode grid (incl. chop)."""
import random
...
def run(assoc, mw, mlast, tmode):
    ...
    for i in SUB:
        ...
        if assoc==0:   w=fmul(fmul(p,asq,64,mw),a,64,mw)
        elif assoc==1: w=fmul(fmul(p,a,64,mw),asq,64,mw)
        else:          w=fmul(p,fmul(asq,a,64,mw),64,mw)
        S=fadd(a,w,64,mlast)
        rows=(168,167,166,165,164,163) if six else (176,175,174,173)
        q=C68(rows[0])
        for row in rows[1:]: q=fadd(fmul(q,asq,64),C68(row),64)
        t=fmul(q,asq,64,tmode)
        m1=fmul(sinT,fadd(ONE,t,200),200); m2=fmul(cosT,S,200)
        sin=fadd(m1,m2,200)
        m3=fmul(cosT,fadd(ONE,t,200),200); m4=fmul(sinT,S,200)
        cos=fadd(m3,(m4[0]^1,m4[1],m4[2]),200)
        if neg and sin[1]: sin=(sin[0]^1,sin[1],sin[2])
        hf=hw[i].split()
        ms+=enc64(sin)!=(int(hf[1],16),int(hf[2],16))
        mc+=enc64(cos)!=(int(hf[3],16),int(hf[4],16))
    return ms,mc
n=len(SUB)
res=[]
for assoc in (0,1,2):
    for mw in ('n','t'):
        for mlast in ('n','t'):
            for tmode in ('n','t'):
                ms,mc=run(assoc,mw,mlast,tmode)
                res.append((ms+mc,assoc,mw,mlast,tmode,ms,mc))
res.sort()
for tot,assoc,mw,mlast,tm,ms,mc in res[:10]:
    print(f"assoc={assoc} w={mw} last={mlast} t={tm}: sin {ms} cos {mc} tot {tot}/{2*n} = {100.0*tot/2/n:.3f}%")

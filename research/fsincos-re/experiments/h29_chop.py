...
def run(mST, mHorner, mAsq):
    ...
    return mm
for mST,mH,mA,label in (('n','n','n',"all-RN (baseline)"),
                        ('t','n','n',"chop S,t only"),
                        ('t','t','n',"chop S,t + Horner"),
                        ('t','t','t',"chop everything"),
                        ('n','t','n',"chop Horner only"),
                        ('n','t','t',"chop Horner+asq"),
                        ('t','n','t',"chop S,t+asq")):
    mm=run(mST,mH,mA)
    print(f"{label:22s}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.3f}%")

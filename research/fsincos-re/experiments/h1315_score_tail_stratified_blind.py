#!/usr/bin/env python3
"""Score the one-shot h1314 FCOS capture against both factor endpoints."""
...
def load_capture(directory: Path) -> dict[tuple[str, str], str]:
    ...
    for mode in CAPTURE_MODES:
        operands = [
            line.strip().lower()
            for line in (directory / f"{mode}_inputs.txt").read_text().splitlines()
            if line.strip()
        ]
        values = []
...
def main() -> None:
    ...
    for row in changes:
        ...
        counts[
            f"q.{row['q']}.cut.{row['cut']}.tail_top4.{row['tail_top4']}."
            f"verdict.{verdict}"
        ] += 1

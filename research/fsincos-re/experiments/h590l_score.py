#!/usr/bin/env python3
"""h590g: cross-schedule 2x2 on the h590k pairs.
...
def main():
    sel = json.load(open("h590k_locked.json"))
    stc = {md: open(f"h590k_{md}_status.txt").read()
           .splitlines() for md in MODES}
    sts = {md: open(f"h590k_sc_{md}_status.txt").read()
           .splitlines() for md in MODES}

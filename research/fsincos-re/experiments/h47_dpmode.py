...
SNAMES=[("f",64,'t',64,'t','t'), ("f",64,'n',64,'n','n')]  # winner + base-factored
...
for pm in ('n','t'):
    for qm in ('n','t'):
        for tm in ('n','t'):
            globals()['PM']=pm; globals()['QM']=qm; globals()['TM']=tm
            score=run_all()
            name=SNAMES[0]
            cs,ns=score[("s","narrow",name)]
            cc,nc=score[("c","narrow",name)]
            print(f"p={pm} q={qm} t={tm}: narrow sin {100.0*cs/max(1,ns):.1f}%  cos {100.0*cc/max(1,nc):.1f}%")

#!/usr/bin/env python3
"""h590f: sc-observable cross-schedule pairs.
...
generated arrangement state.  Locks h590f_locked.json +
h590f_inputs.txt.
"""
import json
...
def feat_fresh(args):
    ...
    if not (-24 <= mb < 24):
        return None
    # sc observability: refs for d=-1,0,1 pairwise distinct
    refs = [tuple(final_cosine_result(-(R + d), ce, md)
    ...
    for pid, (cell, ma, mb2, bk) in enumerate(pairs):
        ...
        for member, mx in (("a", ma), ("b", mb2)):
            recs.append({"pair": pid, "member": member,
                         ...
                         "mb": mb, "st3": st3,
                         "feat": list(bk), "M3": p3})
    json.dump(recs, open("h590f_locked.json", "w"))
    with open("h590f_inputs.txt", "w") as fh:
        for rec in recs:

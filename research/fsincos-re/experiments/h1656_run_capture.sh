#!/bin/sh
# H1656: fresh exception-state input; one attempted instruction, never retried.
# A partial run consumes the entire guard. Preserve it and never restart.
set -eu
[ "$#" -eq 1 ] || exit 2
BIN=$1
...
[ "$(wc -l < hardware-output/state-output.txt)" -eq 36864 ] || exit 3
...
echo 'H1656_CAPTURE_COMPLETE tuples=36864 retries=0'

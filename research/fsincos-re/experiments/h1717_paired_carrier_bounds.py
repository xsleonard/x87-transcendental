...
def main():
    ...
    for e in range(-32,-2):
        ...
        for name in ('S6','C6'):
            state=c[name][6]
            for i in (5,4,3,2):
                state=g.product(state,square,name=name+str(i)+'.product')
                state=g.add(state,c[name][i],name=name+str(i)+'.add')
    ...
    report=dict(experiment='h1717_paired_carrier_bounds',status='PASS_PAIRED_POLYNOMIAL_INTERVAL_BOUNDS',

...
# OTHER, append to act_tail.tsv, delete the chunk.
...
for ((c=0; c<NCH; c++)); do
  ...
  for insn in cos sin; do
    ...
    for mode in rn rd ru rz; do
      ...
      [ $mode = rz ] && RC="--rc=rz"
      paste <(nice -n 10 ./model_payB --batch $RC $FL < chunkT.txt) \
            <(nice -n 10 ./model_payLF --batch $RC $FL < chunkT.txt) \
            chunkT.txt \
        | awk -F'\t' -v i=$insn -v m=$mode -v s=$seed '
            $1 != $2 { print s "\t" i "\t" m "\t" NR-1 "\t" $3 "\t" $1 "\t" $2 }' \
        > tbh_${insn}_${mode}.part &
    done
    wait
  done
  ...
  for insn in cos sin; do
    for mode in rn rd ru rz; do
      f=tbh_${insn}_${mode}.part
      [ -s $f ] || { rm -f $f; continue; }
      cut -f5 $f > tbh_ops.txt
      /root/x87_capture_x86_64 $mode $insn < tbh_ops.txt > tbh_hw.txt
      paste $f tbh_hw.txt | awk -F'\t' '
        {
      ...
      rm -f $f tbh_ops.txt tbh_hw.txt
    done
  done
  echo "CHUNK $c (seed $seed): cumulative $(wc -l < act_tail.tsv 2>/dev/null || echo 0)"
done
rm -f chunkT.txt
echo "PHASEB HITS: $(wc -l < act_tail.tsv)"
cut -f1 act_tail.tsv | sort | uniq -c

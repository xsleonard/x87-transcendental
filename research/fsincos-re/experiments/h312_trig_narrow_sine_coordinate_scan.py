#!/usr/bin/env python3
"""Scan every remaining narrow shared-sine exponent/alignment coordinate."""
...
def main() -> None:
    ...
    for dataset_index, (_, points) in enumerate(selected):
    ...
    for coord, points in records.items():
        for bias in BIASES:
            ...
            changed = 0
            for dataset_index, point, rn64, old_metric in points:
                sine = h79.bias_toward_zero(rn64, bias, 5)
    ...
    ranked.sort()
    print(
        "h312 narrow coordinate scan: "
        f"datasets={len(selected)} bins={len(records)} "

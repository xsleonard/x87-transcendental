#!/usr/bin/env python3
"""Score narrow shared-sine carrier fractions on fresh h307 hardware."""
...
CAPTURE = ROOT / "capture-kit-captures" / "skylake-trig-h307"
CANDIDATES = tuple(range(24, 35))
...
def main() -> None:
    ...
    for index, (se, sig) in enumerate(operands):
        ...
        if point is None:
            raise SystemExit(f"h307 line {index + 1} is not narrow-table")
        points.append(point)
    if any(
        ...
        raise SystemExit("h307 input/capture line counts differ")

    errors = {}
    profiles = {}
    print(f"h308 narrow fractional capture: inputs={len(points)}")
    ...
    for index in range(len(points)):
        ...
        new = errors[29, index]
        if new < old:
            changes["29-improves"] += 1
            changes["removed"] += old - new
        elif new > old:
            changes["29-regresses"] += 1
            changes["added"] += new - old
        elif profiles[29, index] != profiles[32, index]:
            changes["mixed"] += 1
    print(f"  29/256 versus 32/256 changes={dict(changes)}")

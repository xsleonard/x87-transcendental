#!/usr/bin/env python3
"""Prove C/Python parity for the Round-43 shared-sine carrier rule."""
...
import h288_trig_sine_bias_rule as h288
...
FLAGS = (*h228.FLAGS, "--round43-p6-sine-bias")
...
def points(name: str):
    ...
    for index, (se, sig) in enumerate(operands):
        ...
        if point is None:
            raise SystemExit(f"h285 line {index + 1} is not a table point")
        result.append(point)
    return result
...
def main() -> None:
        ...
        for rc in ((args.rc,) if args.rc else h58.RCS):
            ...
            for index, point in by_index.items():
                ...
                expected = tuple(
                    ...
                    for value in h288.hidden_values(point)
                )
                if actual != expected:
                    misses += 1
                    if first is None:
    ...
    print(f"h289 Round-43 C/Python parity: {checks}/{checks} lane-mode checks")

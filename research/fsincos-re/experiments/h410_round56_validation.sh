#!/bin/bash
# h410: final Round-56 revalidation across every hardware corpus.
set -e
...
score fsin_h347 captures/skylake-trig-h347/inputs.txt "captures/skylake-trig-h347/fsin_MODE_status.txt" single "$FSIN"
score fsin_sweep capture-kit/inputs/sweep_inputs.txt "fresh/sweep_fsin_MODE.txt" single "$FSIN"
score fsin_dense capture-kit/inputs/dense_qn.txt "fresh/dense_fsin_MODE.txt" single "$FSIN"
score fsin_h405win h405/window_inputs.txt "h405/hw_sin_MODE.txt" single "$FSIN"
score fsin_h409sep h409/separators.txt "h409/hw_MODE.txt" single "$FSIN"
score fcos_h347 captures/skylake-trig-h347/inputs.txt "captures/skylake-trig-h347/fcos_MODE_status.txt" single "$FCOS"
score fcos_sweep capture-kit/inputs/sweep_inputs.txt "fresh/sweep_fcos_MODE.txt" single "$FCOS"
score fcos_dense capture-kit/inputs/dense_qn.txt "fresh/dense_fcos_MODE.txt" single "$FCOS"
score pair_h347 captures/skylake-trig-h347/inputs.txt "captures/skylake-trig-h347/fsincos_MODE_status.txt" pair "$PAIR"
score pair_h349 captures/skylake-trig-h349/inputs.txt "captures/skylake-trig-h349/fsincos_MODE_status.txt" pair "$PAIR"
score pair_sweep capture-kit/inputs/sweep_inputs.txt "fresh/sweep_fsincos_MODE.txt" pair "$PAIR"
score pair_dense capture-kit/inputs/dense_qn.txt "fresh/dense_fsincos_MODE.txt" pair "$PAIR"
echo "--- h377 gate (expect baseline 1/2):"
python3 experiments/h377_fsin_binary64_residual_gate.py src/fsincos_skylake --expect-baseline | grep "residual gate"
sha256sum src/fsincos_skylake.c src/fsincos_skylake

#!/usr/bin/env python3
"""Independent conditional arithmetic/state algebra and 61-field raw checks.

No H1668/H1669/H1660/H1652 imports. Inconsistent-state conditional hypotheses
...
def main():
    ...
    for index,row in enumerate(rows):
        ...
        words=lines[index].lower().split(); assert len(words)==61 and all(w.count('=')==1 for w in words)
        f=dict(w.split('=') for w in words); assert len(f)==61
        ...
        required={'case','insn','mode','pc','masks','depth','cc','flags','summary','empty','req_sw',
                  'a_valid','fault','fault_at','si_code','trap'}|{p+'_'+k for p in ('a','b','f') for k in state}
        assert set(f)==required
        valid,fault,at,trap=(int(f[k]) for k in ('a_valid','fault','fault_at','trap'))
        assert (valid,fault,at) in ((1,0,0),(0,1,1),(1,1,2))
        ...
        selected='a' if valid else 'f'; sw=int(f['b_sw'],16); actual_sw=int(f[selected+'_sw'],16)
        ...
        assert old['primary_requested_fault_exact']==(bool(row['summary']&128)==(at==1))
    ...
    result=dict(experiment='h1671_independent_summary_state',status='INDEPENDENT_CONDITIONAL_MISSES' if failures else 'PASS_INDEPENDENT_CONDITIONAL_CHECKS',

...
def graph(i, polyP, wbST, combP, finaldirect):
    ...
    return 2-ok
# full-population residual clustering with best config
POS=defaultdict(int); NEG=defaultdict(int); TOT=defaultdict(int)
def graph2(i):
    se,sig=int(inputs[i][:4],16),int(inputs[i][5:21],16)
    neg=(se>>15)&1
    e=(se&0x7FFF)-16383
    ...
    six=b in WIDE
    sinT=C68(SINROW[b]); cosT=C68(COSROW[b])
    a=faddP(rv,(1,b,-6),100)
    asq=fmulP(a,a,100)
    rows=(162,161,160,159,158,157) if six else (172,171,170,169)
    p=C68(rows[0])
    for row in rows[1:]: p=faddP(fmulP(p,asq,100),C68(row),100)
    S=faddP(a,fmulP(fmulP(p,asq,100),a,100),100)
    rows=(168,167,166,165,164,163) if six else (176,175,174,173)
    q=C68(rows[0])
    for row in rows[1:]: q=faddP(fmulP(q,asq,100),C68(row),100)
    t=fmulP(q,asq,100)
    m1=fmulP(sinT,faddP(ONE,t,100),100); m2=fmulP(cosT,S,100)
    sin=faddP(m1,m2,100)
    m3=fmulP(cosT,faddP(ONE,t,100),100); m4=fmulP(sinT,S,100)
    cos=faddP(m3,(m4[0]^1,m4[1],m4[2]),100)
    if neg and sin[1]: sin=(sin[0]^1,sin[1],sin[2])
    hf=hw[i].split()
    for tag,mv,j in (("s",sin,1),("c",cos,3)):
        me=enc64(mv); he=(int(hf[j],16),int(hf[j+1],16))
        TOT[(b,tag)]+=1
        if me!=he:
            up = (he[1]>me[1]) if (he[0]==me[0]) else (he[0]&0x7FFF)>(me[0]&0x7FFF)
            if (he[0]>>15): up = not up
            if up: POS[(b,tag)]+=1
            else: NEG[(b,tag)]+=1
ALL=[]
for i in range(len(inputs)):
    ...
    e=(se&0x7FFF)-16383
    rf=sig*2.0**(e-63)
    if 0.25<=rf<0.7853981633974483: ALL.append(i)
for i in ALL: graph2(i)
print(f"n={len(ALL)}")
for key in sorted(TOT):
    p,ng,t=POS[key],NEG[key],TOT[key]
    if p+ng: print(f"cell {key[0]:2d}/64 {key[1]}: +{p} -{ng}  / {t}  rate={100.0*(p+ng)/t:.3f}%")

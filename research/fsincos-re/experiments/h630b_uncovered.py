#!/usr/bin/env python3
"""h630b: enumerate the uncovered zones from the corpus rescore
(both-branch recovery), with row m's and features."""
import math
...
for fields, hw_sigs in rows:
    ...
    APf = (A2 + P2) << F
    EU = (APf - B_full) >> kf
    ra = [final_cosine_result(-(EU + (0 if side == "up" else -1)),
                              ce, md) for md in ROUNDING_MODES]
    rb = [final_cosine_result(-(EU + (1 if side == "up" else 0)),
                              ce, md) for md in ROUNDING_MODES]
    if ra == rb:
        continue
    mf = (mm & ((1 << 63) - 1)) / (1 << 63)
    ...
    zid = (key, xd12, min(15, int(mf * 16))) if key in \
        V5_KEYS else (key, xd12)
    if FITS.get(zid) is None:
        R = M >> k
        hw = [hw_sigs[md] for md in ROUNDING_MODES]
        bok = hw == [final_cosine_result(-R, ce, md)
                     for md in ROUNDING_MODES]
        unc[(key, xd12, min(15, int(mf * 16)), theta,
             bok)] += 1
print("uncovered zone rows (key, xd12, mf16, theta, base_ok):")
for kk in sorted(unc, key=str):
    print(f"  {kk}: {unc[kk]}")

...
def graph(i, polyP, wbST, combP, finaldirect):
    ...
    if finaldirect:
        sin=rnd(sin[0],sin[1],sin[2],finaldirect); cos=rnd(cos[0],cos[1],cos[2],finaldirect)
    hf=hw[i].split()
    ok=(enc64(sin)==(int(hf[1],16),int(hf[2],16))) + (enc64(cos)==(int(hf[3],16),int(hf[4],16)))
    return 2-ok
res=[]
for f2 in (0,65,66,67,68,69,70,71,72,80):
    mm=sum(graph(i,64,0,WIDEP,f2) for i in SUB)
    res.append((mm,f2))
res.sort()
for mm,f2 in res:
    print(f"double-round@{f2}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.2f}%")

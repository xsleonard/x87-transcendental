#!/usr/bin/env python3
"""h592b prep: ALIAS-ROBUST labels — fire iff hw == refs(EU+1), blind rows (refs(EU)==refs(EU+1)) dropped — emits kf and
Vlow so the borrow-prediction census can reconstruct APf_low and
the true borrow b without replica reruns.
  mhex dist low3 ce side mb fire f4v rfv rsh kf Vlow
"""
from collections import defaultdict
from multiprocessing import Pool
from h437_gate_extraction import ROUNDING_MODES, final_cosine_result
from h577_three_term import qrow3, E2M
from h453_chain_variants import (C6_1, C6_3, C6_5, build_chain,
                                 mul_round)
...
def work(rows):
    ...
    for mhex, hw, R, ce, theta in rows:
        ...
        APf = (A + P) << F
        EU = (APf - B_full) >> kf
        r0 = [final_cosine_result(-EU, ce, md)
              for md in ROUNDING_MODES]
        r1 = [final_cosine_result(-(EU + 1), ce, md)
              for md in ROUNDING_MODES]
        if r0 == r1:
            continue  # blind: fire architecturally unreadable
        if hw == r1:
            req2 = 1
        elif hw == r0:
            req2 = 0
        else:
            continue  # OTHER (should not happen up-side)
        ...
        f4v, rfv = qr[2], qr[3]
        sqv = qr[1]
        mag = (0, E2M, int(mhex, 16))
        sqt = mul_round(mag, mag, 67, "chop")
        f4t = mul_round(sqt, sqt, 67, "chop")
        negv = build_chain(f4t, C6_5, C6_3, C6_1, 67, 64,
                           "rn", False, False, False)[2]
        out.append((mhex, dist, low3, ce, side, mb, fire,
                    f4v, rfv, rsh, kf, Vlow, sqv, negv, lsh))
    return out

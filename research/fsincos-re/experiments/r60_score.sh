...
for corp in comb7 comb9; do
  for mode in rn rd ru; do
    ...
    ./model_r60 --batch $RC $FCOS --round60-fcos-tie-gate \
        < /root/h491/${corp}_inputs.txt > mo_${corp}_${mode}_r60.txt
  done
done
python3 - <<PYEOF
...
for corp in ("comb7", "comb9"):
    ...
    for mode in ("rn", "rd", "ru"):
        ...
        mo = norm(f"mo_{corp}_{mode}_r60.txt")
        for i, (a, b) in enumerate(zip(hw, mo)):
            if a != b:
                tot += 1
                cen[theta.get(inputs[i], "?")] += 1
    print(f"{corp} r60: {tot} mismatches / {3*len(hw)}  by theta: "

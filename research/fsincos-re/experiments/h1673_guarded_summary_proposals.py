...
The guarded protocol records BEFORE_ONLY for unsafe restored summary states.
No skipped state receives instruction-consumption credit. The seed is new.
...
LOCKS={**numerical.LOCKS, **completion.LOCKS,
    ...
    'tmp/ledger33/current/h1672_capture_static_audit/report.json':'e30681855b9fd394e03727f5d070b0b2a9835cff1a6186d44b19502e19aad7cd'}


def proposals():
    rng=random.Random(0x1673); rows=[]; used=set(); serial=0
            ...
            for mode,rcbits in numerical.RC.items():
                for row in rows:
                    ...
                    answer.append(dict(row,case_id=case,instruction=insn,mode=mode,pc=pc,
                        ...
                        identity_predicted_attempted=bool(row['flags']&~row['masks']&63) or row['summary']==0,
...
def main():
    ...
    bank=dict(experiment='h1673_summary_state_proposals',capture_state='SOFTWARE_ONLY_NOT_FROZEN',

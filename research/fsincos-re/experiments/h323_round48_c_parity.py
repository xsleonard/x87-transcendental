#!/usr/bin/env python3
"""Prove C/Python parity for the Round-48 shared-sine carrier rule."""
...
INPUTS = {
    **h228.INPUTS,
    ...
    "h314": ROOT / "capture-kit" / "inputs" / "constraint_trig_narrow_sine_fraction2_h314.txt",
    "h320": ROOT / "capture-kit" / "inputs" / "constraint_trig_narrow_sine_fraction3_h320.txt",
}
FLAGS = (
    *h228.FLAGS,
...
def points(name: str):
    if name not in ("h285", "h292", "h301", "h307", "h314", "h320"):
...
def main() -> None:
        ...
        for rc in ((args.rc,) if args.rc else h58.RCS):
            ...
            for index, point in by_index.items():
                ...
                expected = tuple(
                    ...
                    for value in h322.hidden_values(point)
                )
                if actual != expected:
                    misses += 1
                    if first is None:
    ...
    print(f"h323 Round-48 C/Python parity: {checks}/{checks} lane-mode checks")

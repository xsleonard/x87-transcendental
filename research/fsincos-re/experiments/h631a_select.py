#!/usr/bin/env python3
"""h631a: select capture rows for the uncovered zones.
...
Emit h631_inputs.txt (3ffc m) + h631_rows.json with frame
...
def main():
    ms = set()
    for fn in ("ties_h603.txt", "ties_h604.txt",
               "ties_comb5.txt", "ties_comb8.txt"):
        try:
            for line in open(fn):
                f = line.split()
    ...
    for kk in sorted(cc, key=str):
        print(f"  {kk}: {cc[kk]}")
    json.dump(out, open("h631_rows.json", "w"))
    with open("h631_inputs.txt", "w") as fh:
        for r in out:

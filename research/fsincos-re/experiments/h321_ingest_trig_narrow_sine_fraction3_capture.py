#!/usr/bin/env python3
"""Score the third narrow carrier fraction on fresh h320 hardware."""
...
CAPTURE = ROOT / "capture-kit-captures" / "skylake-trig-h320"
CANDIDATES = tuple(range(27, 41))
...
def main() -> None:
    ...
    for index, (se, sig) in enumerate(operands):
        ...
        if point is None:
            raise SystemExit(f"h320 line {index + 1} is not narrow-table")
        points.append(point)
    if any(
        ...
        raise SystemExit("h320 input/capture line counts differ")

    print(f"h321 third narrow fractional capture: inputs={len(points)}")

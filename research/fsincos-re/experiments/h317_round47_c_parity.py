#!/usr/bin/env python3
"""Prove C/Python parity for the Round-47 shared-sine carrier rule."""
...
def points(name: str):
    if name not in ("h285", "h292", "h301", "h307", "h314"):
...
def main() -> None:
        ...
        for rc in ((args.rc,) if args.rc else h58.RCS):
            ...
            for index, point in by_index.items():
                ...
                expected = tuple(
                    ...
                    for value in h316.hidden_values(point)
                )
                if actual != expected:
                    misses += 1
                    if first is None:
    ...
    print(f"h317 Round-47 C/Python parity: {checks}/{checks} lane-mode checks")

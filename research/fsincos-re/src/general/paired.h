...
static void general_paired_polynomial(wv_t magnitude, int residual_sign,
    int64_t signed_n, sf_rc_t rc, x80_t out[2], int c1[2])
{
    ...
    p = p5_wv_mul_round(p, square, 67, P5_ROUND_CHOP);
    p = h1630_add(p, h1630_literal(sine[0]), 64, P5_ROUND_RN);
    q = p5_wv_mul_round(q, square, 67, P5_ROUND_CHOP);
    ...
    for (int lane = 0; lane < 2; ++lane) {
        ...
        sf_t result = general_paired_final(lead, tails[cosine_branch], negative, rc, &c1[lane]);
        sf_to_x87(&result, &out[lane].se, &out[lane].sig);
    }
}

static fsincos_status_t general_paired_ref(
    ...
    if ((in.se & 0x7fff) && !(in.sig >> 63)) {
        ...
        if (x.cls == SF_NAN) {
            ...
            if (tiny) {
                ...
                if (top < -2) {
                    path = "polynomial";
                    general_paired_polynomial(residual, residual_sign, signed_n, rc, out, c1);
                } else {
                    path = "table";
                    for (int lane = 0; lane < 2; ++lane) {

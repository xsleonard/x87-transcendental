"""Append D0057's completed input-only challenge without changing old packs."""
...
def main():
    ...
    previous_path = corpus / 'CATALOG-D0046.json'
    ...
    for name, pack in packs.items():
        ...
        if name == 'd0057':
            assert counts['rows'] == complete['rows'] == 2616
            pack['counts'] = counts
        else:
    ...
    pool_path = output / 'visible-even-inner-tie-input-pool.tsv.gz'
    rows = 0
    with gzip.open(pool_path, 'xt') as target:
        for name in ('d0056-node1-pilot', 'd0056-node1-wide'):
    ...
    save(output / 'MANIFEST.json', dict(status='APPEND_ONLY_CPU_INDEPENDENT_INPUT_EXTENSION',
        name='d0057', pack=packs['d0057'], pool=pool,
    ...
    save(corpus / 'CATALOG-D0057.json', dict(format='fpatan-corpus-v1-additive-catalog',

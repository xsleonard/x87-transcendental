"""Append D0063's completed input-only challenge without changing old packs."""
...
def main():
    ...
    previous_path = corpus / 'CATALOG-D0061.json'
    ...
    for name, pack in packs.items():
        ...
        if name == 'd0063':
            assert counts['rows'] == complete['rows'] == 872
            pack['counts'] = counts
        else:
    ...
    pool_path = output / 'visible-long-odd-tie-input-pool.tsv.gz'
    rows = 0
    with gzip.open(pool_path, 'xt') as target:
        for name in ('d0062-long-prefix2',):
    ...
    save(output / 'MANIFEST.json', dict(status='APPEND_ONLY_CPU_INDEPENDENT_INPUT_EXTENSION',
        name='d0063', pack=packs['d0063'], pool=pool,
    ...
    save(corpus / 'CATALOG-D0063.json', dict(format='fpatan-corpus-v1-additive-catalog',

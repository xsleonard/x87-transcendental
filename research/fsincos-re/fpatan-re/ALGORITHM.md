# FPATAN fixed numerical reconstruction (V7)

**Status:** V7 is integrated into the main C implementation after 624,312
prospective observations with zero misses. It matches all 2,783,208 retained
observations. V4, V5 and V6 were falsified; their records remain historical.
See [DELIVERY.md](DELIVERY.md) and [ACCEPTANCE.md](ACCEPTANCE.md).

For a code-first specification, read [PSEUDOCODE.md](PSEUDOCODE.md).
The [standalone FPATAN manuscript](paper/README.md) includes the same
pseudocode, literal constants, equations and confirmed evidence.

This is the single graph in `fpatan_candidate.c`, not a set of optional
patches. The arithmetic roles and bit widths below are constrained by
Skylake observations; the physical implementation of hidden microcode has
not been recovered. See the handoff for exact verification scope and misses.
...
Canonical both-special pairs conservatively held by the history audit are
documented-domain/synthetic checks, not claimed native captures. The
independent Goldmont operation listing supports the interleaved polynomial
structure and distinct operation classes, but not a physical Skylake decode.
All current recorded misses are closed; the evidence is extensive empirical
validation, not exhaustive testing of every raw80 pair or CPU revision.

...
from prepare_d0063 import witnesses
from protocol import make_line
import score_d0046_ties as scorer


class LongOddTieDiscriminatorTests(unittest.TestCase):
    def test_all_external_witnesses_and_both_parities(self):
        rows = witnesses()
        self.assertEqual(len(rows), 3)
        self.assertEqual(Counter(r['parity'] for r in rows), {0: 2, 1: 1})

    def test_scorer_detects_both_possible_hardware_outcomes(self):
        base = Path(tempfile.mkdtemp(prefix='fpatan-d0063-synthetic-'))
        for mutate in (False, True):
            ...
            for parity in (0, 1):
                ...
                overrides.append(json.dumps(dict(id=ident, node=0, parity=parity,
            ...
            save(job / 'STRUCTURAL-HYPOTHESIS.json', dict(
                ...
                counts={'node0:separators:parity0': 1, 'node0:separators:parity1': 1}))
            ...
            self.assertEqual(counts['node0:nearest-even:union_misses'], 2 * int(mutate))
            self.assertEqual(counts['node0:nearest-odd:union_misses'], 2 * int(not mutate))
            self.assertEqual(counts['node0:ties-away:union_misses'], 1)
            self.assertEqual(counts['node0:ties-zero:union_misses'], 1)

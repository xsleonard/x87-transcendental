...
def main():
    output = BASE / 'd0061-verification.json'
    ...
    assert complete['rows'] == manifest['rows'] == score['counts']['rows'] == rules['counts']['rows'] == 2904
    ...
    assert manifest['counts']['selected_pairs'] == manifest['counts']['generated_pairs'] == 720
    ...
    for name in ('C-PREFLIGHT.json', 'C-SANITIZED-PREFLIGHT.json'):
        ...
        assert preflight['rows'] == 2904 and preflight['C_frozen_Python_differences'] == 0
    ...
    assert any(row[1:] == ['d0061', 'OBSERVED', 2904] for row in ledger['compressed_batches'])
    assert read(job / 'SSH_CAPTURE_RETURN.json')['returncode'] == 0
    counts = rules['counts']
    assert counts['node2:separators:parity0'] == hypothesis['counts']['node2:separators:parity0'] == 18
    assert counts['node2:separators:parity1'] == hypothesis['counts']['node2:separators:parity1'] == 14
    identified = counts['baseline_union_misses'] == 0
    if identified:
        for rule, expected in (('nearest-even', 0), ('nearest-odd', 32), ('ties-away', 18), ('ties-zero', 14)):
            assert counts[f'node2:{rule}:union_misses'] == expected
    ...
    catalog_path = HERE / 'corpus-v1/CATALOG-D0061.json'
    catalog = read(catalog_path)
    assert catalog['counts']['rows'] == 7125168 and len(catalog['packs']) == 21
    assert catalog['exact_tuple_duplicates'] == 0
    assert catalog['previous_catalog']['sha256'] == digest(HERE / 'corpus-v1/CATALOG-D0057.json')
    for pack in catalog['packs'].values():
        assert digest(HERE / 'corpus-v1' / pack['path']) == pack['sha256']
    assert catalog['extension_manifest_sha256'] == digest(HERE / 'corpus-v1/extensions/d0061/MANIFEST.json')
    ...
    tests = subprocess.run([sys.executable, str(HERE / 'test_d0061_discriminators.py')], text=True, capture_output=True)
    ...
    paths += [catalog_path, filter_receipt, BASE / 'd0059-short-wide-replay/REPORT.json', BASE / 'd0058-filter-failure.json', BASE / 'd0057-verification.json']
    previous = read(BASE / 'd0057-verification.json')
    assert previous['status'] == 'PASS_AUTHENTICATED_TIE_DISCRIMINATION_AUDIT'
    resolved = dict(previous['identified_among_fixed_rules'])
    assert resolved == {'table:correction_sum': 'nearest-even', 'direct:even_inner_sum': 'nearest-even'}
    if identified:
        resolved['table:odd_sum'] = 'nearest-even'
    unresolved = ['direct:odd_inner_sum']
    if not identified:
        unresolved.append('table:odd_sum')
    save(output, dict(status='PASS_AUTHENTICATED_TIE_DISCRIMINATION_AUDIT',
        ...
        new_skylake_rows=2904, previous_two_context_rows=7097584, corpus_rows=7125168,

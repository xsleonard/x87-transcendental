"""Read-only audit of public research-host files for prior FPATAN usage.

Run BEFORE uploading any FPATAN capture job. Does not scan unrelated projects
...
def main():
    roots=sorted(p for p in Path('/root').iterdir() if
        (p.is_dir() and (p.name.startswith('fsincos') or re.fullmatch(r'h\d+.*',p.name))))
    ...
    def scan(stream,name):
        ...
        while True:
            ...
            if not b:break
            byte_count+=len(b)
            if b'fpatan' in (tail+b).lower():hits.append(name);break
            tail=b[-6:]
    suffixes={'.c','.h','.py','.sh','.md','.txt','.json','.jsonl','.log','.tsv','.csv','.out','.gz','.xz','.bz2','.zip','.tgz','.tar'}
    for root in roots:
        for path in sorted(root.rglob('*')):
            if not path.is_file() or path.is_symlink():continue
            if 'fpatan' in path.name.lower():hits.append(str(path)+' [filename]')
    ...
    print(json.dumps(dict(status='NO_PRIOR_FPATAN_FOUND' if not hits and not unread else 'REVIEW_REQUIRED',
        roots=[str(p) for p in roots],files=files,bytes_scanned=byte_count,hits=hits,unread=unread,

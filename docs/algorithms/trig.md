...
This walkthrough describes the Skylake `FSIN`, `FCOS` and `FSINCOS` algorithms
implemented in [sin_cos.c](../../src/trig/sin_cos.c) and
[sincos.c](../../src/trig/sincos.c). For other instructions, see the
[algorithm index](README.md). The
[unified LaTeX paper](../../research/fsincos-re/paper/x87-suite.tex) includes the same walkthrough in
...
[`p5_rom_constants.h`](../../src/constants/trig.c), including the
...
Start with the evaluator at the top of [sin_cos.c](../../src/trig/sin_cos.c)
for FSIN/FCOS, or [sincos.c](../../src/trig/sincos.c) for FSINCOS. Each follows
the classification, reduction and dispatch above, with its polynomial below.
The shared [table](../../src/trig/table.c) and [tiny-result](../../src/trig/tiny.c)
kernels receive an already reduced argument. [reduce.c](../../src/trig/reduce.c)
implements exact reduction, and [wide.c](../../src/arithmetic/wide.c) implements
the shared integer arithmetic and precision cuts. See the [source map](../../SOURCE.md)
for public entry points and exception handling.

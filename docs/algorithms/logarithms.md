...
The C source uses bounded finite integer arithmetic with every precision cut
explicit; the earlier GMP reference was removed after successful validation.
The separately written [Python reference](../../tests/reference/log_reference.py) uses exact fractions.
Neither implementation executes x87, calls a host logarithm, or looks up saved
input/output pairs. See [ACCEPTANCE.md](../../research/fsincos-re/fyl2x-re/ACCEPTANCE.md) for measured coverage.
...
The [C API](../../include/x87trans/x87trans.h) returns raw80 result bits, C1 and the six arithmetic

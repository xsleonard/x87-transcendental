"""Check the corrected short-odd filter on every saved unfiltered H carry."""
import argparse
...
from d0059_short_odd_boundary import parameters, block_enclosure
...
def main():
    ...
    tests = subprocess.run([sys.executable, str(HERE / 'test_d0059_short_boundary.py')],
                           text=True, capture_output=True)
    assert tests.returncode == 0, tests.stderr
    failure = BASE / 'd0058-filter-failure.json'
    assert json.loads(failure.read_text())['status'] == 'FAIL_FALSE_NEGATIVE_EXCLUSION'
    pins[str(failure)] = digest(failure)
    for name in ('d0059_verify_filter.py', 'd0059_short_odd_boundary.py', 'test_d0059_short_boundary.py',
                 'd0058_short_odd_boundary.py', 'd0056_outer_boundary_lattice.py'):
        pins[str(HERE / name)] = digest(HERE / name)
    save(args.out, dict(status='PASS_CORRECTED_SHORT_ODD_FILTER_AUDIT', counts=counts,
        ...
        limits='Every saved H carry in three independently generated unfiltered scans survives four block-size versions of the corrected filter and full Fraction replay. The failed D0058 exclusion and counterexample are retained. These finite checks support the interval derivation, not an all-input hardware proof.'))

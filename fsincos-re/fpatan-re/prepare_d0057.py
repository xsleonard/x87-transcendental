"""Freeze both-parity, endpoint-visible long even-inner tie discriminators."""
...
MINING = (BASE / 'd0056-node1-pilot', BASE / 'd0056-node1-wide')
...
def witnesses():
    ...
    for directory in MINING:
        ...
        assert report['status'] == 'VERIFIED_BOUNDARY_FILTERED_TIE_SEARCH'
        ...
        for row in report['witnesses']:
            assert row['status'] == 'EXACT_EXTERNAL_ENDPOINT_SEPARATOR' and row['node'] == 1
            ...
            event = next(r for r in records if r[0] == 'even_inner_sum')
            assert event[3:5] == ('tie', row['parity'])
            second, _ = traced_kernel(state['z'], False,
                ('even_inner_sum', 'ties-away' if row['parity'] == 0 else 'ties-zero'))
        ...
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                ...
                for raw in orbits((*a, *b)):
                    yield raw, f'node1:seed{index}:parity{row["parity"]}:y{dy}:x{dx}'
...
def freeze_controls(job):
    ...
    assert all(counts[f'node1:separators:parity{p}'] for p in (0, 1))
    save(job / 'STRUCTURAL-HYPOTHESIS.json', dict(status='FROZEN_ENDPOINT_VISIBLE_TIE_CONTROLS',
        target_nodes=[1], candidate_rules=['nearest-even', 'nearest-odd', 'ties-away', 'ties-zero'],
...
if __name__ == '__main__':
    freeze('d0057', 'd0057-long-even-inner-visible-ties-20260906', generate,
        ('prepare_d0057.py', 'd0056_outer_boundary_lattice.py', 'd0049_algebraic_tie_preimages.py',
        ...
        purpose='Exact long even-inner RN64 tie witnesses at both retained parities, full 3x3 operand neighborhoods and all sign/octant/RC orbits')
    freeze_controls(BASE / 'd0057')

"""Freeze both-parity, endpoint-visible short odd-chain tie discriminators."""
...
def witnesses():
    ...
    for directory in MINING:
        ...
        assert report['status'] == 'VERIFIED_TRUNCATED_SHORT_ODD_BOUNDARY_SEARCH'
        assert digest(directory / 'targets.tsv') == report['target_sha256']
        assert digest(directory / 'blocks.tsv') == report['blocks_sha256']
        for source, sha in report['source_sha256'].items():
            assert digest(HERE / source) == sha
        for row in report['witnesses']:
            assert row['status'] == 'EXACT_EXTERNAL_ENDPOINT_SEPARATOR' and row['node'] == 2
            ...
            assert state['path'] == 'table' and state['z'] == row['sign'] * int(row['z_sig'], 16) * audit.two(row['z_step'])
            first, records = traced_kernel(state['z'], True)
            event = next(r for r in records if r[0] == 'odd_sum')
            assert event[3:5] == ('tie', row['parity'])
            second, _ = traced_kernel(state['z'], True,
                ('odd_sum', 'ties-away' if row['parity'] == 0 else 'ties-zero'))
            ...
            assert restored(state, first)[0] == audit.SPEC['finite_angle'](y, x)
        ...
        for dy in (-1, 0, 1):
            for dx in (-1, 0, 1):
                ...
                for raw in orbits((*a, *b)):
                    yield raw, f'node2:seed{index}:parity{row["parity"]}:y{dy}:x{dx}'
...
def freeze_controls(job):
    ...
    assert all(counts[f'node2:separators:parity{p}'] for p in (0, 1))
    save(job / 'STRUCTURAL-HYPOTHESIS.json', dict(status='FROZEN_ENDPOINT_VISIBLE_TIE_CONTROLS',
        target_nodes=[2], candidate_rules=['nearest-even', 'nearest-odd', 'ties-away', 'ties-zero'],
...
if __name__ == '__main__':
    freeze('d0061', 'd0061-short-odd-chain-visible-ties-20260906', generate,
        ('prepare_d0061.py', 'd0059_short_odd_boundary.py', 'd0058_short_odd_boundary.py', 'd0056_outer_boundary_lattice.py', 'd0049_algebraic_tie_preimages.py',
        ...
        purpose='Exact short odd-chain RN64 tie witnesses at both retained parities, full 3x3 operand neighborhoods and all sign/octant/RC orbits')
    freeze_controls(BASE / 'd0061')

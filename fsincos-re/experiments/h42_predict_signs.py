...
def S_variants(a,asq,p):
    ...
    out["S chop"]=fadd(a,w,64,'t')
    # factored: u=1+p*asq ; S=a*u
    u_rn=fadd(ONE,fmul(p,asq,64),64)
    u_ch=fadd(ONE,fmul(p,asq,64),64,'t')
    out["fact uRN Srn"]=fmul(a,u_rn,64)
    out["fact uCH Srn"]=fmul(a,u_ch,64)
    out["fact uRN Sch"]=fmul(a,u_rn,64,'t')
    out["fact uCH Sch"]=fmul(a,u_ch,64,'t')
    return out

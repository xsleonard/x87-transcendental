...
def parse_output(line: str, status=False):
    if status:
        match = re.fullmatch(r"(OK ([0-9a-f]{4}) ([0-9a-f]{16})|C2) SW ([0-9a-f]{4})", line, re.IGNORECASE)
        assert match, line
        sw = int(match[4], 16)
        if match[1].lower() == "c2":
            assert sw & 0x400
            return "C2", sw
        return ("C2" if sw & 0x400 else match[2].lower() + ":" + match[3].lower()), sw
    if line == "C2":
        return "C2"
...
def main() -> None:
    # Preserve hexadecimal payloads and accept status-only C2 rows.
    assert parse_output("OK 3fff 8000000000000000 SW 3820", True) == ("3fff:8000000000000000", 0x3820)
    assert parse_output("OK 3FFE ABC2ABC2ABC2ABC2 SW 3A20", True) == ("3ffe:abc2abc2abc2abc2", 0x3a20)
    assert parse_output("C2 SW 3c00", True) == ("C2", 0x3c00)
    ...
    result = {"experiment": "h1627_wider_cosine_transfer_v2", "complete_banks": reports,

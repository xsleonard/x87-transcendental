...
def mkS(name,a,asq,p):
    kind,Pm,mm,Pu,mu,ms=name
    m=fmul(p,asq,Pm,mm)
    if kind=="f":
        u=fadd(ONE,m,Pu,mu)
        return fmul(a,u,64,ms)
    else:   # unmaterialized u: S = round(a*(1+m)) = round(a + a*m) one rounding
        am=fmul(a,m,200)
        return fadd(a,am,64,ms)
SNAMES=[]
for kind in ("f","g"):
    for Pm,mm in ((64,'n'),(64,'t'),(65,'t'),(66,'t'),(67,'t')):
        for Pu,mu in ((64,'t'),(65,'t'),(66,'t'),(64,'n')):
            for ms in ('t','n'):
                if kind=="g" and (Pu,mu)!=(64,'t'): continue
                SNAMES.append((kind,Pm,mm,Pu,mu,ms))
...
for l,mrow,h in zip(inputs,meta,hw):
    ...
    if abs(int(dstr)) > (1<<(int(usstr)-8)): continue   # sharp subset only
    b,sinT,cosT,a,asq,p,t=parts(sig64,e)
    # boundary from stored d/us relative to the base model value
    Sb=fadd(a,fmul(fmul(p,asq,64),a,64),64)   # base model for boundary calc
    if side=="s":
...
# rank by combined narrow performance (sin + cos)
rank=[]
for name in SNAMES:
    cs,ns=score[("s","narrow",name)]
    cc,nc=score[("c","narrow",name)]
    ws,nws=score[("s","wide",name)]
    wc,nwc=score[("c","wide",name)]
    if ns and nc:
        rank.append((cs/ns+cc/nc, name, 100.0*cs/ns, 100.0*cc/nc,
                     100.0*ws/max(1,nws), 100.0*wc/max(1,nwc)))
rank.sort(reverse=True)
print("top by narrow sin+cos:  (kind,Pm,mm,Pu,mu,ms) nsin ncos wsin wcos")
for sc,name,a_,b_,c_,d_ in rank[:12]:
    print(f"  {name}: {a_:.1f} {b_:.1f} | {c_:.1f} {d_:.1f}")

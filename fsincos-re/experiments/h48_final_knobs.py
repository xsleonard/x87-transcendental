...
def parts(sig64,e):
    ...
    a=fadd(rv,(1,b,-6),200)
    asq=fmul(a,a,64,ASQM)
    rows=S6 if six else S4
    p=C68(rows[0])
    for row in rows[1:]: p=fadd(fmul(p,asq,64,PM),C68(row),64,PM)
    rows=C6 if six else C4
    q=C68(rows[0])
    for row in rows[1:]: q=fadd(fmul(q,asq,64,QM),C68(row),64,QM)
    t=fmul(q,asq,64,TM)
...
SNAMES=[("f",64,'t',64,'t','t'), ("g",64,'t',64,'t','t'), ("g",64,'n',64,'t','t')]
...
for asqm in ('n','t'):
    ...
    for name in SNAMES:
        ...
        print(f"asq={asqm} {name}: nsin {100.0*cs/max(1,ns):.1f}% ncos {100.0*cc/max(1,nc):.1f}% "
              f"wsin {100.0*ws/max(1,nws):.1f}% wcos {100.0*wc/max(1,nwc):.1f}%")

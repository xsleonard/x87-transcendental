#!/usr/bin/env python3
"""P(hw_up | frac(u)) curves: read hardware's chop point and offset off the
sharp tiebreaker data directly."""
...
for l,mrow,h in zip(inputs,meta,hw):
    ...
    t=fmul(q,asq,64)
    # S_wide = a + (p*asq)*a with 64-bit chain (base) kept WIDE at the end
    w=fmul(fmul(p,asq,64),a,64)
    Sw=fadd(a,w,300)                     # wide S value
    sS,gS,ES=Sw
    # fixed-point fraction of |S| on the candidate grid 2^-GRID
    GRID=66
    sh=ES+GRID                            # |S| * 2^GRID = gS * 2^(ES+GRID)
    if sh>=0: v=gS<<sh
    else: v=gS>>(-sh)
    fr=(v&((1<<0).__class__(0)))*0.0
    # fraction = v mod 1 in units: bits below 2^0 => low "..." — v is integer part*frac...
    # value*2^GRID has fractional part = bits below binary point:
    if sh>=0:
        fr=0.0                            # exact multiple (no fraction) — cannot happen for real S
    else:
        frac_bits=gS&((1<<(-sh))-1)
        fr=frac_bits/float(1<<(-sh))
    # base-model boundary + hw side (same as before)
    ...
    agree=(mse,sg_)==(int(h[1],16),int(h[2],16))
    hw_up=(d>0) if agree else (not (d>0))
    # a sign: positive half or negative half of cell
    apos = a[0]==0
    fb=min(15,int(fr*16))
    curve[(b>=36,"a+" if apos else "a-",fb)][0]+= 1 if hw_up else 0
    curve[(b>=36,"a+" if apos else "a-",fb)][1]+=1
print("P(hw_up) vs frac(u_wide) [16 bins], sin side, sharp points")
for wide in (False,True):
    for asgn in ("a-","a+"):
        row=[]
        for fb in range(16):
            c,n=curve[(wide,asgn,fb)]
            row.append(f"{100.0*c/n:3.0f}({n:3d})" if n else "  -(  0)")
        print(f"{'wide ' if wide else 'narrow'} {asgn}: "+" ".join(row))

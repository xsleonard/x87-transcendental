...
from h1717_promoted_regression import SOURCE, HEADER
...
def main():
    ...
    archived=original/'all_O2';assert digest(archived)==old['sha256']['binaries']['all_O2']
    ...
    # The independent-review separator and its signed neighbors are mandatory
    # software controls; this does not create new hardware observations.
    for sign in (0,0x8000):
        for delta in range(-16,17):
            operands.add(f'{0x3ffc|sign:04x} {0xe79000000c3e46e7+delta:016x}')
    ...
    for label,flags in configs.items():
        ...
        for mode in ('rn','rd','ru','rz'):
            ...
            for i,op in enumerate(operands):
                expected,metadata=independent.expected(op,mode,'all',cache)
    ...
    report=dict(experiment='h1717_verify_promotion',status='PASS_PROMOTED_IMPLEMENTATION',operands=len(operands),

#!/usr/bin/env python3
"""Refine the 65-bit power carrier with fixed Horner-add formats.

h1334's best global arithmetic schedule uses square=odd65 and
fourth=chop65, followed by the recovered Horner chain.  h1335 shows that
changing either Horner-product representation cannot reduce its seven
classification errors.  Holding the power carrier and products fixed, this
pass exhausts every independent 60--80-bit chop/RN/away/odd representation
for add1 and add2.
...
def main() -> None:
    ...
    scores = []
    exact = []
    for add1_width in range(60, 81):
        for add1_mode in MODES:
            for add2_width in range(60, 81):
                for add2_mode in MODES:
                    widths = list(BASE_WIDTHS)
                    modes = list(BASE_MODES)
                    widths[0], modes[0] = 65, "odd"
                    widths[1], modes[1] = 65, "chop"
                    widths[3], modes[3] = add1_width, add1_mode
                    widths[5], modes[5] = add2_width, add2_mode
                    candidate = Candidate(
                        "horner_add_grid",
                        f"add1.{add1_mode}{add1_width}."
                        f"add2.{add2_mode}{add2_width}",
                    ...
                    item = (
                        ...
                        add1_width, add1_mode, add2_width, add2_mode,
                    )
                    scores.append(item)
                    if not item[0]:
                        exact.append(item)
    ...
    with args.report.open("x") as output:
        ...
        output.write(
            "candidate_policy\tfixed_odd65_chop65_power_plus_horner_add_grid\n")
        ...
        output.write(
            ...
            "delta_zero\tdelta_other\tadd1_width\tadd1_mode\t"
            "add2_width\tadd2_mode\n")

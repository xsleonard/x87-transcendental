#!/bin/bash
# rv13 promotion blind: seed 0x872E, 4M ops, 8 legs; incumbent =
...
set -e
cd /root/r84
/root/r84/probe_ledger.sh rv13-pre
python3 /root/r59/h733_gen.py 0x872E 4000000 rv13_inputs.txt
wc -l rv13_inputs.txt
gcc -O2 -ffp-contract=off -o rv13_incumbent /root/r59/fsincos_skylake.c -lm
gcc -O2 -ffp-contract=off -I/root/r59 -DG_R96TOPCLOSED=1 -DG_R96ACTCLOSED=1 -o rv13_candidate /root/r84/fsincos_skylake.c -lm
echo RV12_BUILDS_OK
for insn in cos sin; do
  for mode in rn rd ru rz; do
    ( /root/x87_capture_x86_64 $mode $insn < rv13_inputs.txt \
        > rv13_hw_${insn}_${mode}.txt ) &
  done
  wait
  echo "RV12_CAPTURED $insn"
done
/root/r84/probe_ledger.sh rv13-post
rm -f rv13_report.txt
for insn in cos sin; do
  FL="--fcos-standalone"; [ $insn = sin ] && FL="--fsin-standalone"
  for mode in rn rd ru rz; do
    RC=""; [ $mode = rd ] && RC="--rc=rd"; [ $mode = ru ] && RC="--rc=ru"
    [ $mode = rz ] && RC="--rc=rz"
    paste <(nice -n 10 ./rv13_incumbent --batch $RC $FL < rv13_inputs.txt) \
          <(nice -n 10 ./rv13_candidate --batch $RC $FL < rv13_inputs.txt) \
          rv13_hw_${insn}_${mode}.txt rv13_inputs.txt \
      | awk -F"\t" -v i=$insn -v m=$mode '
          {
            ...
            if (as!=bs) {
              ...
              print "DIFF\t" i "\t" m "\t" NR-1 "\t" $4 "\t" tag "\t" as "\t" bs "\t" hs >> "rv13_report.txt";
            }
          }
...
[ -f rv13_report.txt ] && awk -F"\t" "{print \$6}" rv13_report.txt | sort | uniq -c || echo "0 diff rows"
echo RV12_DONE

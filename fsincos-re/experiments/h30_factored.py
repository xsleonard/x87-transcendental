...
def run(mST, mHorner, mAsq):
    ...
    for i in SUB:
        ...
        if FACTORED:
            u=fadd(ONE,fmul(p,asq,64,mHorner),64,mST)   # u = 1 + a^2*P
            S=fmul(a,u,64,SMUL)                          # S = a*u
        else:
            S=fadd(a,fmul(fmul(p,asq,64,mHorner),a,64,mHorner),64,mST)
...
for fac in (0,1):
    for mu in ('n','t'):
        for smul in ('n','t'):
            if not fac and (mu!='n' or smul!='n'): continue
            globals()['FACTORED']=fac; globals()['SMUL']=smul
            mm=run(mu,'n','n')
            res.append((mm,fac,mu,smul))
res.sort()
for mm,fac,mu,smul in res:
    print(f"factored={fac} u_round={mu} smul={smul}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.3f}%")

...
def run(HP, fuse_sin, fuse_cos, mulstage):
    ...
    for i in SUB:
        ...
        if fuse_sin==0:
            ...
            sin=fadd(r,w,200)
        elif fuse_sin==2:              # r + (RN64(p*rsq))*r fused final
            w=fmul(p,rsq)
            sin=fadd(r,fmul(w,r,200),200)
        else:                          # deeper: Horner to c5; fused r + rsq*r*(c3 + rsq*prest)
            prest=C68(S6[5])
            for row in (S6[4],S6[3],S6[2],S6[1]): prest=fadd(fmul(prest,rsq,HP),C68(row),HP)
            # fused: r + c3*(rsq*r) + prest*(rsq^2*r) -- all exact inside
            rc=fmul(rsq,r,200); r5=fmul(rc,rsq,200)
            t1=fmul(C68(S6[0]),rc,200); t2=fmul(prest,r5,200)
            sin=fadd(fadd(r,t1,200),t2,200)
        q=C68(C6[5])
        ...
        else:                          # deeper: fused 1 + c2*rsq + qrest*rsq^2
            qrest=C68(C6[5])
            for row in (C6[4],C6[3],C6[2],C6[1]): qrest=fadd(fmul(qrest,rsq,HP),C68(row),HP)
            rq=fmul(rsq,rsq,200)
            u1=fmul(C68(C6[0]),rsq,200); u2=fmul(qrest,rq,200)
            cos=fadd(fadd(ONE,u1,200),u2,200)

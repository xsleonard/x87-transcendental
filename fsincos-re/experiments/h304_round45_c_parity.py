#!/usr/bin/env python3
"""Prove C/Python parity for the Round-45 shared-sine carrier rule."""
...
def main() -> None:
        ...
        for rc in ((args.rc,) if args.rc else h58.RCS):
            ...
            for index, point in by_index.items():
                ...
                expected = tuple(
                    ...
                    for value in h303.hidden_values(point)
                )
                if actual != expected:
                    misses += 1
                    if first is None:
    ...
    print(f"h304 Round-45 C/Python parity: {checks}/{checks} lane-mode checks")

#!/usr/bin/env python3
"""S,t materialized at 64; products m1..m4 rounded at P; final add -> 64."""
import random
...
def run(PM, onePlusT):
    """PM: product precision; onePlusT: materialize (1+t) at 64 first?"""
    mm=0
    for i in SUB:
        se,sig=int(inputs[i][:4],16),int(inputs[i][5:21],16)
        neg=(se>>15)&1
        ...
        t=fmul(q,asq,64)
        # delta-accumulation: u = RN_P(fused(sinT*t + cosT*S)); sin = RN64(sinT+u)
        u1=fmul(sinT,t,100); u2=fmul(cosT,S,100)
        u=fadd(u1,u2,PM)
        sin=fadd(sinT,u,100)
        v1=fmul(cosT,t,100); v2=fmul(sinT,S,100)
        v=fadd(v1,(v2[0]^1,v2[1],v2[2]),PM)
        cos=fadd(cosT,v,100)
        if onePlusT: pass
    ...
    return mm
for onePlusT in (0,1):
    for PM in (100,70,69,68,67,66,65):
        mm=run(PM,onePlusT)
        print(f"onePlusT@64={onePlusT} prodP={PM}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.3f}%")

#!/usr/bin/env python3
"""h632b: fit remaining uncovered zones on the h632 captures and
...
held-out by m-hash; report.  Output: h632_model_v7.json.
"""
import json
...
def load_rows():
    recs = json.load(open("h632_rows.json"))
    st_f = {md: open(f"h632_{md}_status.txt").read()
    ...
    for i, rec in enumerate(recs):
        ...
        key = tuple(rec["key"])
        zid = ((key, rec["xd12"], rec["mf16"])
               if key in V5_KEYS else (key, rec["xd12"]))
        out.append((zid, half, rec["tau"], rec["mf"], jlo,
                    jhi, rec["st"]))
    return out
...
def main():
    ...
    for zid, f in fits.items():
        if f is None:
            continue
        tgt = v5 if len(zid) == 3 else v3
        key = str(zid)
        if key not in tgt["fits"]:
            tgt["fits"][key] = list(f)
            if len(zid) == 3:
                a5 += 1
            else:
    ...
    for (zid, st), c in cbest.items():
        ...
        tgt = v5 if len(zid) == 3 else v3
        key = str((zid, st))
        if key not in tgt["cbest"]:
            tgt["cbest"][key] = c
            if len(zid) == 3:
                c5 += 1
            else:
                c3 += 1
    json.dump(v3, open("h632_model_v3v7.json", "w"))
    json.dump(v5, open("h632_model_v5v7.json", "w"))
    print(f"v7: v3 +{a3} zones +{c3} offsets; "
          f"v5 +{a5} zones +{c5} offsets")

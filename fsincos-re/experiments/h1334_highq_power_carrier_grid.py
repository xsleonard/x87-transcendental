#!/usr/bin/env python3
"""Search independent fixed square/fourth carrier representations.

h1333 localizes h1332's strongest arithmetic approximation to the first two
power-building products: odd65 at both square and fourth misses only eight
of 189 labels, while either Horner multiply is irrelevant or harmful.  This
pass exhausts every fixed 60--80-bit chop/RN/away/odd representation for the
square and fourth independently, leaving the Horner chain at its recovered
chop67/RN64 schedule.

This is a global arithmetic grid, not an operand-conditioned selector.
...
def main() -> None:
        ...
        for square_mode in MODES:
            for fourth_width in range(60, 81):
                for fourth_mode in MODES:
                    ...
                    widths[0], widths[1] = square_width, fourth_width
                    modes[0], modes[1] = square_mode, fourth_mode
                    name = (
                        f"square.{square_mode}{square_width}."
                        f"fourth.{fourth_mode}{fourth_width}")
                    candidate = Candidate(
                        "power_grid", name, tuple(widths), tuple(modes))
                    counts = Counter()
                    ...
                    item = (
                        ...
                        square_width, square_mode, fourth_width, fourth_mode,
                    )
                    scores.append(item)
                    if not item[0]:
                        exact.append(item)
    ...
    with args.report.open("x") as output:
        ...
        output.write("candidate_policy\tfixed_square_fourth_width_mode_grid\n")
        ...
        output.write(
            ...
            "delta_zero\tdelta_other\tsquare_width\tsquare_mode\t"

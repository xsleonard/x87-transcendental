...
EPREC=68; KPREC=68
def storeP(s,sig,E,k,mode='n'):
    b=sig.bit_length(); sh=b-k
    if sh<=0: return (s,sig,E)
    top=sig>>sh; rem=sig&((1<<sh)-1)
    if mode=='n':
        half=1<<(sh-1)
    ...
    return (s,top,E+sh)
def C68(row):
    ef,s,sig=ROM[row]; return storeP(s,sig,(ef-0xFFFD)-68,globals()['KPREC'])
def CENT(row):
    ef,s,sig=ROM[row]; return storeP(s,sig,(ef-0xFFFD)-68,globals()['EPREC'])
...
def graph(i, polyP, wbST, combP, finaldirect):
    ...
    six=b in WIDE
    sinT=CENT(SINROW[b]); cosT=CENT(COSROW[b])
...
import sys
res=[]
for ep in (68,67,66,65,64):
    for kp in (68,67,66,65,64):
        globals()['EPREC']=ep; globals()['KPREC']=kp
        # rebind in module scope
        import builtins
        mm=sum(graph(i,64,0,WIDEP,0) for i in SUB)
        res.append((mm,ep,kp))
res.sort()
for mm,ep,kp in res[:12]:
    print(f"entries@{ep} coeffs@{kp}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.2f}%")

...
def run(mST, mHorner, mAsq):
    ...
    for i in SUB:
        ...
        S=fadd(a,fmul(fmul(p,asq,64,mHorner),a,64,mHorner),SWID,'n')
...
SWID=64; TWID=64
res=[]
for sw in (64,65,66,67,68):
    for tw in (64,65,66,67,68):
        globals()['SWID']=sw; globals()['TWID']=tw
        mm=run('n','n','n')
        res.append((mm,sw,tw))
res.sort()
for mm,sw,tw in res[:10]:
    print(f"S@{sw} t@{tw}: {mm}/{2*len(SUB)} = {100.0*mm/2/len(SUB):.3f}%")

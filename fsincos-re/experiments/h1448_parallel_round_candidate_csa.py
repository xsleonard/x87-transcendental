#!/usr/bin/env python3
"""Audit Intel's redundant plus-one round-candidate path at R59.

Intel US6055555A computes both representable neighbors of a wide arithmetic
result in parallel.  Its second path injects a one at the least-significant
retained position directly into the unresolved sum/carry tuple with a 3:2
carry-save adder, then resolves that new tuple with a carry-propagate adder.
This is a different physical representation from adding one to an already
resolved and truncated value.

This audit composes that literal interface with the exact 67-by-64-bit P5
radix-8/4:2 multiplier tree reconstructed from Intel US5195051A.  For x87
extended precision, the retained significand is 64 bits.  At each of the
eight recovered FMUL sites and in both operand-port orientations, the script
adds the retained-grid unit to the raw P5 sum/carry words and asserts that the
new redundant tuple resolves to the exact product plus that unit.

Only the source-defined output words of the plus-one 3:2 CSA are scored.  The
two Intel disclosures make this a physically plausible isomorphism, not
evidence that Skylake uses this representation.  There are no operand
identities, thresholds, learned trees, new hardware observations, or emulator
changes.
"""
...
ROUND_PATENT = "US6055555A"
ROUND_URL = "https://patents.google.com/patent/US6055555A/en"
...
def csa3(left: int, right: int, addend: int) -> tuple[int, int]:
    """Return a 3:2 CSA tuple with carry bits in result columns."""
    total_sum = (left ^ right ^ addend) & PRODUCT_MASK
    carry = (
...
def candidate_state(
    ...
    sum_word = state["sum"] & PRODUCT_MASK
    carry_word = state["carry"] & PRODUCT_MASK
    product = (sum_word + carry_word) & PRODUCT_MASK
    ...
    leading_position = product.bit_length() - 1
    retain_shift = leading_position - (RETAINED_BITS - 1)
    if retain_shift not in (66, 67):
        raise RuntimeError(
            f"unexpected normalized retained-grid position {retain_shift}"
        )
    retained_unit = 1 << retain_shift
    candidate_sum, candidate_carry = csa3(
        sum_word, carry_word, retained_unit
    )
    candidate = (candidate_sum + candidate_carry) & PRODUCT_MASK
    expected = (product + retained_unit) & PRODUCT_MASK
    if candidate != expected:
        raise RuntimeError("plus-one CSA did not reconstruct rounded neighbor")

    retained_mask = (1 << RETAINED_BITS) - 1
    lower = (product >> retain_shift) & retained_mask
    upper = (candidate >> retain_shift) & retained_mask
    if upper != ((lower + 1) & retained_mask):
        raise RuntimeError("parallel paths are not adjacent grid points")

    values: dict[str, int] = {
        "retained_unit.column66": int(retain_shift == 66),
        "retained_unit.column67": int(retain_shift == 67),
    }
    for bit in range(PRODUCT_BITS):
        values[f"candidate_sum.bit{bit}"] = (candidate_sum >> bit) & 1
        values[f"candidate_carry.bit{bit}"] = (candidate_carry >> bit) & 1
...
def source_signals(
    ...
    for stage, operands in h1441.fmul_inputs(row).items():
        for role in ROLES:
            local, trace = candidate_state(*operands, role)
            stem = f"{stage}.{role}"
            values.update({
                f"{stem}.{name}": value for name, value in local.items()
            })
...
def prepare(args: argparse.Namespace) -> tuple[
    ...
    for row_index, item in enumerate(base):
        ...
        if names is None:
            ...
            raise RuntimeError("parallel-candidate signal schema changed")
...
def main() -> None:
    ...
    with args.report.open("x") as output:
        ...
        output.write(
            f"round_interface_primary_source\t{ROUND_PATENT}\t{ROUND_URL}\t"
            "source_vendor=Intel\n"
        )
        output.write(
        ...
        output.write(
            "candidate_policy\tplus_one_3to2_CSA_output_words_only_"
            "no_operand_fitting\n"
        )
        output.write(
        ...
        output.write(
            "parallel_candidate_identity\tcandidate_sum_plus_candidate_carry_"
            "mod_2^131=exact_product_plus_retained_grid_unit\n"
        )
        output.write(f"retained_significand_bits\t{RETAINED_BITS}\n")
        output.write("retained_unit_columns\t66,67\n")
        ...
        output.write("\n[joint parallel-candidate-history collisions]\n")
        output.write(
        ...
        output.write("\n[target parallel-candidate traces]\n")
        ...
        if gate_exact or pair_exact:
            output.write("parallel_round_candidate_selector\tCANDIDATE_ONLY\n")
        else:
            output.write("parallel_round_candidate_selector\tno_exact_program\n")

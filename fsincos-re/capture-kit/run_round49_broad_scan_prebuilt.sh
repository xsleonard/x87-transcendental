#!/bin/sh
# Compiler-free paired FSINCOS capture for the million-input Round-49 scan.
set -eu
cd "$(dirname "$0")"
...
INPUT=inputs/round49_broad_scan_h349.txt
OUT=${1:-round49-broad-scan-out}
[ -r "$INPUT" ] || {
    echo "missing input file: $INPUT"
    echo "generate it with experiments/h349_round49_broad_scan_inputs.py"
    exit 1
}
mkdir -p "$OUT"
